...
**KNN regression** is a non-parametric regression method that attempts to predict the value $x_{0}$ corresponding to the given set of predictors by identifying the *K* training observations that are closet to $x_{0}$, represented by $\mathit{N}_{0}$. KNN regression then estimates $\mathit{f}(x_{0})$ using the average of all the training responses in $\mathit{N}_{0}$. In other words, $$\mathit{\hat{\mathit{f}}}(x_{0})= \frac{1}{\mathit{K}}\sum_{x_{i\in\mathit{N_{0}}}}\mathit{y_{i}}$$
